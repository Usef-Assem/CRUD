    ...
    for( var i = 0 ; i < productList.length ; i++){
        var temp = ""
            ...
            for(var i =0 ; i < productList.length ; i++){
                ...
                || productList[i].Category.toLowerCase().includes(searchVal) == true || productList[i].Desc.toLowerCase().includes(searchVal) == true )
             temp += `  <tr>
             <td> `+ i +` </td>
             ...
             <td> `+productList[i].Desc.toLowerCase().replace(searchVal , "<span class = 'bg-danger'>"+searchVal+"</span>")+`</td>
             <td><button onclick= "update(` + i + `)" class="btn btn-warning btn-sm">Update</button></td>
             <td><button onclick="deleteProduct(` + i + `)" class="btn btn-danger btn-sm">Delete</button></td>
         </tr> `
            } 
